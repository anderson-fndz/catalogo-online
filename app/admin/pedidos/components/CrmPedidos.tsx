...
import { Loader2, Users, Filter, Plus, Calendar, Scissors, Edit, Trash2, BusFront, DollarSign, Image as ImageIcon } from "lucide-react";
...
export default function CrmPedidos() {
  ...
  const carregarDados = async () => {
    ...
    const [resClientes, resProdutos, resHistorico] = await Promise.all([
      ...
      supabase.from("produtos").select("id, nome, tecido, preco, cores, grade_tamanhos, imagens").order("nome", { ascending: true }),
      supabase.from("historico_compras").select(`id, cor, tamanho, quantidade, data_compra, preco_unitario, grupo_id, cliente:clientes(id, nome, whatsapp, estado, nome_pacote), produto:produtos(id, nome, tecido, imagens)`).order("data_compra", { ascending: false })
    ]);
  ...
  const abrirWhatsApp = (nome: string, zap: string, tecidos: string[]) => {
    ...
    if (tecidos.length === 1) tecidoStr = tecidos[0];
    else if (tecidos.length === 2) tecidoStr = `${tecidos[0]} e ${tecidos[1]}`;
    else if (tecidos.length > 2) tecidoStr = `${tecidos[0]}, ${tecidos[1]} e outros modelos`;
  ...
  const pedidosAgrupados = Object.values(historicoFiltrado.reduce((acc: any, item: any) => {
    ...
    if (!acc[key]) acc[key] = { id: key, grupo_id: item.grupo_id, cliente: item.cliente, data_compra: item.data_compra, itens: [], total_pecas: 0, valor_total: 0, tecidos: new Set(), modelos: new Set(), imagens: new Set() };
    
    acc[key].itens.push(item); 
    acc[key].total_pecas += item.quantidade; 
    acc[key].valor_total += (item.quantidade * (item.preco_unitario || 0));
    
    if (item.produto?.tecido) acc[key].tecidos.add(item.produto.tecido); 
    if (item.produto?.nome) acc[key].modelos.add(item.produto.nome);
    
    if (item.produto?.imagens && Array.isArray(item.produto.imagens) && item.produto.imagens[0]) {
      acc[key].imagens.add(item.produto.imagens[0]);
    }
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
      <div className="bg-card border border-border/60 rounded-xl shadow-sm overflow-hidden">
        ...
        <div className="overflow-x-auto">
          {/* ADICIONADO table-fixed E min-w PARA BLINDAR O ALINHAMENTO */}
          <table className="w-full min-w-[1000px] text-left border-collapse table-fixed">
            <thead>
              <tr className="border-b border-border/50 bg-secondary/5 text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
                {/* LARGURAS EXATAS (em porcentagem) DEFINIDAS PARA CADA COLUNA */}
                <th className="p-4 pl-6 w-[22%]"><div className="flex items-center gap-1.5"><Users size={14}/> Cliente</div></th>
                <th className="p-4 w-[45%]"><div className="flex items-center gap-1.5"><Scissors size={14}/> Resumo do Pedido</div></th>
                <th className="p-4 w-[13%]"><div className="flex items-center gap-1.5"><Calendar size={14}/> Data Pgto</div></th>
                <th className="p-4 w-[12%] text-right"><div className="flex items-center justify-end gap-1.5"><DollarSign size={14}/> Total</div></th>
                <th className="p-4 w-[8%] text-center pr-6">Ação</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border/50 text-sm">
              {pedidosAgrupados.length === 0 ? (
                ...
                pedidosAgrupados.map((grupo: any) => {
                  const imagensArray = Array.from(grupo.imagens);
                  return (
                    <tr key={grupo.id} className="hover:bg-secondary/5 transition-colors group">
                      
                      {/* CLIENTE */}
                      <td className="p-4 pl-6 align-top">
                        <div className="font-bold text-foreground text-base line-clamp-1">{grupo.cliente?.nome}</div>
                        <div className="text-[10px] text-muted-foreground font-medium mt-0.5">{grupo.cliente?.whatsapp}</div>
                        <div className="flex items-center gap-2 mt-2">
                          <span className="bg-secondary/50 text-foreground border border-border/50 px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-widest">{grupo.cliente?.estado || 'N/I'}</span>
                        </div>
                      </td>
                      
                      {/* RESUMO DO PEDIDO COM CAIXA FIXA E LIMITADOR DE TEXTO */}
                      <td className="p-4 align-top">
                        <div className="flex items-center gap-4">
                          {/* CAIXA IMAGINÁRIA FIXA PARA AS FOTOS: w-[160px] */}
                          <div className="flex -space-x-3 shrink-0 relative z-0 w-[160px]">
                            {imagensArray.slice(0, 3).map((img: any, idx: number) => (
                              <img key={idx} src={img} alt="Peça" className="w-12 h-12 rounded-lg object-cover border-2 border-background shadow-sm" />
                            ))}
                            {imagensArray.length > 3 && (
                              <div className="w-12 h-12 rounded-lg bg-secondary flex items-center justify-center text-[10px] font-bold border-2 border-background text-foreground z-10 shadow-sm">
                                +{imagensArray.length - 3}
                              </div>
                            )}
                            {imagensArray.length === 0 && (
                              <div className="w-12 h-12 rounded-lg bg-secondary/50 flex items-center justify-center text-muted-foreground border-2 border-background shadow-sm">
                                <ImageIcon size={20} />
                              </div>
                            )}
                          </div>
                          
                          {/* TEXTO BLINDADO: min-w-0 flex-1 */}
                          <div className="min-w-0 flex-1">
                            <div className="font-extrabold text-sm mb-1">{grupo.total_pecas} Peças</div>
                            <div className="text-[10px] text-muted-foreground uppercase tracking-widest leading-relaxed line-clamp-2" title={Array.from(grupo.modelos).join(', ')}>
                              {Array.from(grupo.modelos).join(', ')}
                            </div>
                            <div className="flex gap-1.5 mt-1.5 flex-wrap">
                              {Array.from(grupo.tecidos).map((t: any) => <span key={t} className="text-[9px] text-primary bg-primary/5 border border-primary/20 px-1.5 py-0.5 rounded-md uppercase tracking-widest font-bold">{t}</span>)}
                            </div>
                          </div>
                        </div>
                      </td>
                      ...
                      {/* DATA */}
                      <td className="p-4 text-xs font-bold text-muted-foreground uppercase tracking-wider align-top">
                        {format(new Date(grupo.data_compra), "dd MMM yy", { locale: ptBR })}
                        <div className="text-[9px] mt-1 text-amber-600 font-extrabold">{differenceInDays(new Date(), new Date(grupo.data_compra))} dias atrás</div>
                      </td>

                      {/* TOTAL */}
                      <td className="p-4 text-right font-black text-foreground text-base align-top">
                        R$ {grupo.valor_total.toFixed(2).replace('.', ',')}
                      </td>

                      {/* AÇÕES */}
                      <td className="p-4 text-center pr-6 align-top">
                        <div className="flex items-center justify-center gap-2">
                          <button onClick={() => abrirWhatsApp(grupo.cliente?.nome, grupo.cliente?.whatsapp, Array.from(grupo.tecidos))} className="bg-[#25D366] hover:bg-[#1ebd5a] text-white p-2.5 rounded-xl shadow-sm transition-all hover:scale-110">
                            <WhatsappIcon size={18} />
                          </button>
                          <button onClick={() => { setGrupoEdicao(grupo); setIsModalAberto(true); }} className="bg-secondary/80 hover:bg-secondary text-foreground p-2.5 rounded-xl border border-border/50 transition-colors">
                            <Edit size={16} />
                          </button>
                        </div>
                      </td>
